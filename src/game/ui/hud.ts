...
export class HUD {
  ...
  constructor(private game: Game, onSkill: (slot: number) => void) {
    ...
    gridLayout.forEach(({ slot: i, row, col }) => {
      ...
      btn.innerHTML = `<span class="ic" data-emoji="${meta?.icon ?? ''}" data-skill-id="${meta?.id ?? ''}">${skillIconHtml(meta?.icon ?? '·', 30, meta?.id)}</span><canvas class="cd-arc" width="62" height="62"></canvas><div class="cd"></div><div class="nm">${meta?.name ?? '空'}</div>`;
  ...
  update(): void {
    ...
    this.skills.forEach((s) => {
      ...
      if (s.ic.dataset.emoji !== icon || s.ic.dataset.skillId !== (key?.id ?? '')) {
        s.ic.dataset.emoji = icon;
        s.ic.dataset.skillId = key?.id ?? '';
        s.ic.innerHTML = skillIconHtml(icon, 30, key?.id);
      }
